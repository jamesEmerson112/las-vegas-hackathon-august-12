...
const App: React.FC = () => {
  ...
  const handleTicTacToeMove = (position: number) => {
    const newGameState = TicTacToeLogic.makeMove(ticTacToeGameState, position);
    setTicTacToeGameState(newGameState);

    // Update stats if game is over
    if (newGameState.isGameOver) {
      setTicTacToeStats(prevStats =>
        TicTacToeLogic.updateStats(prevStats, newGameState)
      );
    }
  };

  const handleNewTicTacToeGame = () => {
    setTicTacToeGameState(TicTacToeLogic.createInitialState());
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {activeGame === 'tictactoe' && (
          <TicTacToeGame
            ...
            onNewGame={handleNewTicTacToeGame}
            onResetStats={handleResetTicTacToeStats}
          />
        )}
